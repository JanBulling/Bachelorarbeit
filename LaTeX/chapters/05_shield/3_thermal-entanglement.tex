...
The generation of entanglement between two particles depends significantly on variations in the particle-shield separation, as discussed in \cref{cha:entanglement-generation}.
A vibrating shield effectively alters the separation and angle of the cat-state relative to it, as depicted in \cref{fig:5:vibrating-translation-to-variations}.
\begin{figure}[!htbp]
  \centering
...
\end{figure}
This approximation is only valid for shields significantly larger than the particle radius ($r_s \gg R$) and for low vibrational frequencies ($1/\omega \approx t_\mathrm{max}$), effectively capturing the impact of the first vibrational modes and especially the mode $(1,0)$. 
...
Contrary to the problem considered in \cref{cha:entanglement-generation}, in this case only the thermal amplitude $z_{kl}$ is an independent random variable distributed around $\mean{z_{kl}}=0$ with a standard deviation $\Delta z_{kl}$ given by eq. \eqref{eq:5:amplitude-variance}. 
Variations in particle-shield separation ($\Delta L$) and angle ($\theta$) are correlated to the vibration amplitude $z$.
