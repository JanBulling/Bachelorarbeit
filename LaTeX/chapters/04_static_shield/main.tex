...
\end{figure}
The superpositions are extended in arbitrary orientations $\alpha$ and $\beta$ (due to symmetries, it is enough to consider $\alpha, \beta \in [0, \pi)$). Most notably, the orientation of $\alpha = \beta = 0$ represents the same \q{parallel configuration} as discussed earlier. The other extreme case $\alpha = \beta = \pi/2$, I will call the \q{orthogonal configuration}.
